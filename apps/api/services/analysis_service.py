...
async def submit_full_pipeline(
    ...
    with get_sync_session() as session:
        ...
        # video_path may be a full path, storage:// URI, or plain filename — extract basename
        import os as _os

        filename = _os.path.basename(video_path)
        source_uri = f"storage://projects/{proj_id}/videos/{vid}/source/{filename}"
        video_repo.create(
            video_id=vid,
            project_id=proj_id,
    ...
    # Copy source file to new video's source directory
    import os as _os
    import shutil as _shutil

    storage_root = _os.getenv("STORAGE_ROOT", "./data")
    dest_dir = _os.path.join(storage_root, "projects", proj_id, "videos", vid, "source")
    _os.makedirs(dest_dir, exist_ok=True)
    dest_path = _os.path.join(dest_dir, filename)

    # Resolve original file path
    if video_path.startswith("storage://"):
        orig_path = _os.path.join(storage_root, video_path[len("storage://") :])
    elif _os.path.isabs(video_path):
        orig_path = video_path
    else:
        orig_path = (
            video_path if _os.path.exists(video_path) else _os.path.join(storage_root, video_path)
        )

    if _os.path.exists(orig_path) and not _os.path.exists(dest_path):
        _shutil.copy2(orig_path, dest_path)
        ...
        try:
            ...
            if source_uri.startswith("storage://"):
                ...
                video_path_full = (
                    ...
                    else os.path.join(storage_root, source_uri)
                )
            ...
            if not os.path.exists(normalized_path):
                ...
                validate_normalization(
                    probe_before=probe_before, probe_after=probe_after, output_path=normalized_path
                )
            else:
                probe_after = run_ffprobe(normalized_path)
            ...
            with get_sync_session() as session:
                task_repo = TaskRepository(session)
                task_repo.update_progress(task_id, 40, stage="extract_keyframes")
                session.commit()
            ...
            segs = []  # Whisper/Doubao ASR removed — not needed currently
            with ThreadPoolExecutor(max_workers=2) as pool:
                if not kf_done:
                    targets = compute_keyframe_targets(
                        shots_list, probe_after.fps_num, probe_after.fps_den
                    )
                    kf_future = pool.submit(
                        lambda: extract_keyframes(
                            normalized_path, targets, Path(kf_proxy), max_long_side=320, quality=85
                        )
                    )
            ...
            if not scene_analysis:
                with get_sync_session() as session:
                    ...
                    task_repo.update_progress(task_id, 100, stage="complete")
                    session.commit()
                return

            with get_sync_session() as session:
                task_repo = TaskRepository(session)
                task_repo.update_progress(task_id, 60, stage="scoring")
                session.commit()

            # Step 5+6: VLM ∥ DeepSeek (parallel, skip if scores exist)
            vlm_scores = []
            plot_scores = []
            vlm_scores_path = os.path.join(
                storage_root,
                artifact_base,
                "vlm_boundary",
                "0.1.0",
                "location_character_scores.json",
            )
            plot_scores_path = os.path.join(
                storage_root, artifact_base, "deepseek_plot", "1.0.0", "plot_scores.json"
            )
            ...
            with ThreadPoolExecutor(max_workers=2) as pool:
                ...
                if plot_done:
                    ...
                    sub_lines = [
                        f"[{int(s['start_ms'] // 60000):02d}:"
                        f"{(s['start_ms'] % 60000) / 1000:05.2f}] {s['text'][:80]}"
                        for s in segs
                    ]
                    ...
                    if api_key:
                        ...
                        dprompt = (
                            '你是电影情节分析助手。以下是电影字幕时间线。请规划叙事事件（大/中/小三层）。大事件(major)：类似剧本的\'幕\'。中事件(medium)：大事件内的阶段。小事件(minor)：中事件内的节拍。每个事件输出label,level,time_range(起止ms)。只输出JSON：{"events":[{"label":"...","level":"major","time_range":{"start_ms":0,"end_ms":600000}}]}\n---\n'
                            + "\n".join(sub_lines)
                        )
                ...
                if plot_future:
                    ...
                    if not events:
                        try:
                            events = json.loads(raw_txt).get("events", [])
                        except Exception:
                            pass
                    level_map = {"major": 100, "medium": 60, "minor": 30}
                    for shot in shots_list[:-1]:
                        ...
                        for evt in events:
                            ...
                            if es <= bms < ee:
                                continue
                            if abs(bms - es) < 500:
                                sc = level_map.get(evt.get("level", "minor"), 30)
                                if sc > mx:
                                    mx = sc
                        if mx > 0:
                            plot_scores.append({"shot_id": shot["shot_id"], "plot_change": mx})
            ...
            # Compute weights
            if score_mode == "location_only":
                w = (1.0, 0.0, 0.0)
            elif score_mode == "character_only":
                w = (0.0, 1.0, 0.0)
            elif score_mode == "plot_only":
                w = (0.0, 0.0, 1.0)
            elif score_mode == "custom":
                total = float(location_weight + character_weight + plot_weight)
                w = (
                    (location_weight / total, character_weight / total, plot_weight / total)
                    if total > 0
                    else (1.0, 0.0, 0.0)
                )
                ...
                merged.append(
                    {
                        ...
                        "plot_change_score": pp.get("plot_change", 0),
            ...
            for b in selected:
                ...
                scene_start = (
                    shots_list[ni]["start_ms"] if ni < len(shots_list) else b["timestamp_ms"]
                )
                scene_start_shot = (
                    shots_list[ni]["shot_id"] if ni < len(shots_list) else b["shot_id"]
                )
            final_scenes.append(
                {
                    "start_shot": scene_start_shot,
            ...
            with open(os.path.join(merge_dir, "final_result.json"), "w") as f:
                json.dump(
                    {"final_scenes": final_scenes, "candidate_boundaries": candidate_boundaries}, f
                )
            ...
            with get_sync_session() as session:
                task_repo = TaskRepository(session)
                task_repo.set_error(task_id, "PIPELINE_FAILED", str(e)[:500])
                session.commit()

    threading.Thread(target=run_pipeline, daemon=True).start()

    msg = "normalize → detect → keyframes"
    if scene_analysis:
        msg += " → transcribe → vlm+plot → merge"
    msg += " → complete"
    ...
    return {
        ...
        "progress": 5,
        "message": f"Pipeline started: {msg}",
    }
