...
from models.vlm_boundary.prompts import LOCATION_CHARACTER_SYSTEM, LOCATION_CHARACTER_BATCH_TEMPLATE
...
class VLMSceneBoundaryAdapter(BaseModelAdapter):
    ...
    def unload(self):
        self._provider = None; self._loaded = False
    ...
    def predict(self, model_input: dict[str, Any]) -> dict[str, Any]:
        ...
        tid = model_input["task_id"]; vid = model_input["video_id"]
        ...
        try:
            ...
            with open(sp) as f: shots = json.load(f).get("shots", [])
            n = len(shots)
            if n < 2: return self._e(tid, vid, sv, "TOO_FEW_SHOTS", f"Need>=2, got {n}", False)
            kb = self._d(kf_dir)
            bounds = []
            for i in range(n - 1):
            ...
            t0 = time.monotonic(); all_s = []
            for bs0 in range(0, len(bounds), bs):
                batch = bounds[bs0:bs0+bs]
                uc = [{"type":"text","text": LOCATION_CHARACTER_BATCH_TEMPLATE.format(batch_size=len(batch), shot_ids=", ".join(b["shot_id"] for b in batch))}]
                for b in batch:
                    for fp in [b["end_frame"], b["start_frame"]]:
                        if os.path.exists(fp): uc.append({"type":"image_url","image_url":{"url":encode_image_jpeg(fp)}})
                resp = self._provider.send([{"role":"system","content":LOCATION_CHARACTER_SYSTEM},{"role":"user","content":uc}])
            ...
            return self._ok(tid,vid,sv,"location_character_scores",f"storage://{art}",len(all_s),rt)
        except Exception as e:
            return self._e(tid,vid,sv,"VLM_INFERENCE_FAILED",str(e),True)

    @staticmethod
    def _r(u): p="storage://"; return os.path.join(os.getenv("STORAGE_ROOT","./data"),u[len(p):]) if u.startswith(p) else u
    @staticmethod
    def _d(u): return VLMSceneBoundaryAdapter._r(u)
    @staticmethod
    def _ok(t,v,s,k,u,c,m): return {"schema_version":s,"task_id":t,"video_id":v,"status":"SUCCEEDED","model":{"name":"vlm_scene_boundary","version":"0.1.0"},"artifacts":{k:u},"metrics":{"score_count":c,"runtime_ms":m},"error":None}
    @staticmethod
    def _e(t,v,s,c,m,r): return {"schema_version":s,"task_id":t,"video_id":v,"status":"FAILED","model":{"name":"vlm_scene_boundary","version":"0.1.0"},"artifacts":{},"metrics":{},"error":{"code":c,"message":m,"retryable":r}}
