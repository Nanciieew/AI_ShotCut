...
if not os.path.exists(_FFPROBE):
    import shutil; shutil.copy2(_FFMPEG, _FFPROBE)
...
def main() -> int:
    ...
    import omnishotcut
    model = omnishotcut.load(str(PROJECT_ROOT / "model_store/omnishotcut/1.0.0/OmniShotCut_ckpt.pth"))

    validator = FrameDiffValidator(mad_threshold=5.0, hist_threshold=0.95)
    results = []

    for vp in videos:
        vname = vp.name; vpath = str(vp)
        ...
        if raw and len(raw) > 1:
            ...
            if fp_count > 0:
                print(f"  [FRAME-DIFF] Filtered {fp_count} boundary(s), {len(raw)}→{len(filtered)} shots")
        ...
        print(f"  fps={num}/{den} dur={dur}s runtime={rt:.1f}s shots_raw={raw_count} shots_filtered={filt_count} {status}")

        entry = {
            "video": vname, "fps_num": num, "fps_den": den, "fps": round(num/den,4),
            "frame_count": fc, "duration_s": dur, "mode": "clean_shot",
            "runtime_s": round(rt, 2), "error": err,
            "shot_count_raw": raw_count, "shot_count_filtered": filt_count,
            "raw_ranges": raw, "confidences": confs,
            "filtered_ranges": filtered,
            "frame_diff_stats": fd_stats,
            "frame_diff_boundaries": [
                {"frame": b.frame_idx, "mad": b.mad, "hist_corr": b.hist_corr,
                 "flagged": b.is_likely_false} for b in fd_report.boundaries
            ] if fd_report else [],
        }
    ...
    for r in results:
        ...
        print(f"{r['video']:<30} {r['shot_count_raw']:>5} {r['shot_count_filtered']:>5} {r['runtime_s']:>7.1f}s {status}")
    print(f"\nSummary: {summary}")
    return 0
