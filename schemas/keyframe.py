...
class KeyframeSample(BaseModel):
    ...
    position_num: int = Field(..., ge=0, description="Numerator of position fraction (1=25%, 1=50%, 3=75%)")
    ...
    decoded_pts_ms: Optional[int] = Field(default=None, ge=0, description="PTS from decoder (validation)")
    ...
    duplicated_reference: bool = Field(default=False, description="True if this sample shares an image with another sample")
...
class ShotKeyframes(BaseModel):
    ...
    samples: list[KeyframeSample] = Field(..., min_length=1, max_length=3, description="1-3 keyframe samples (always 3 requested; dedup may reduce unique count)")
...
class KeyframeSummary(BaseModel):
    ...
    producer: dict = Field(..., description="{name, version, backend, pyav_version, ffmpeg_version}")
    source: dict = Field(..., description="{normalized_video_artifact_id, shots_artifact_id, fps_num, fps_den, frame_count}")
    ...
    metrics: dict = Field(default_factory=dict, description="{shot_count, requested_sample_count, unique_image_count, deduplicated_sample_count, total_bytes, runtime_ms}")
