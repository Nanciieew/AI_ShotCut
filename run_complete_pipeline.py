...
import json, os, sys, time, shutil, re, base64, uuid
...
for line in env.splitlines():
    if line.startswith("QWEN_VL_API_KEY="):
        QWEN_KEY = line.split("=", 1)[1].strip(); break
...
def step(name, fn):
    ...
    try:
        ...
        elapsed = time.monotonic() - t0
        log.append({"step": name, "status": "FAILED", "elapsed_s": round(elapsed, 1), "error": str(e)[:200]})
        print(f"  FAILED: {e}")
        return None
...
def check_existing():
    ...
    return {"shots": n_shots, "keyframes": n_kf, "shots_path": str(shots_path), "norm_path": str(norm_path)}
...
def extract_audio():
    ...
    import subprocess
    subprocess.run([
        "ffmpeg", "-y", "-i", str(DATA / "video_normalization/1.0.0/normalized.mp4"),
...
def transcribe():
    ...
    a = WhisperAdapter(); a.load()
    r = a.predict({"task_id":"t1","video_id":VID,"model":{"name":"whisper","version":"1.0.0"},"input":{"audio_uri":str(audio_path)},"parameters":{}})
    assert r["status"] == "SUCCEEDED", r.get("error")
    segs = r["artifacts"]["subtitle_segments"]
    json.dump({"video_id":VID,"segments":segs}, open(subs_path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
...
def score_vlm():
    ...
    a = VLMSceneBoundaryAdapter(); a.load()
    all_scores = []
    ...
    for bi in range(0, len(shots) - 1, bs):
        ...
        try:
            r = a.predict({"task_id":f"b{bi}","video_id":VID,"model":{"name":"vlm","version":"0.1.0"},"input":{"shots_uri":tmp,"keyframes_dir":kf},"parameters":{"batch_size":bs}})
            if r["status"] == "SUCCEEDED":
                scores = a._last_result.get("scores", [])
                all_scores.extend(scores)
                print(f"  Batch {bi//bs+1}: {len(scores)} scores (total {len(all_scores)})")
            else:
                print(f"  Batch {bi//bs+1}: FAILED — {r.get('error',{}).get('message','')[:80]}")
        except Exception as e:
            print(f"  Batch {bi//bs+1}: ERROR — {e}")
        finally:
            if os.path.exists(tmp): os.remove(tmp)
    json.dump({"video_id":VID,"scores":all_scores}, open(qwen_out,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
...
def score_plot():
    ...
    prompt = f"""你是电影情节分析助手。以下是电影字幕时间线。请规划叙事事件（大/中/小三层）。大事件(major)：类似剧本的"幕"。中事件(medium)：大事件内的阶段。小事件(minor)：中事件内的节拍。每个事件输出label,level,time_range(起止ms)。只输出JSON：{{{{"events":[{{{{"label":"...","level":"major","time_range":{{{{"start_ms":0,"end_ms":600000}}}}}}}}]}}}}---字幕（{len(subtitles)}句，{subtitles[-1]['end_ms']//60000}分钟）---\n""" + "\n".join(sub_lines)
    ...
    if not events:
        try: events = json.loads(raw_text).get("events", [])
    ...
    for i, shot in enumerate(shots[:-1]):
        ...
        for evt in events:
            ...
            if abs(bms - es) < 500:
                ...
                if sc > mx: mx = sc
        if mx > 0: scores.append({"shot_id": shot["shot_id"], "plot_change": mx})
    json.dump({"video_id":VID,"events":events,"plot_scores":scores}, open(plot_out,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
...
def merge():
    ...
    plot = {p["shot_id"]:p for p in json.load(open(plot_out,encoding="utf-8")).get("plot_scores",[])}
    W = (0.35, 0.35, 0.30)
    scenes, start, start_sid = [], shots[0]["start_ms"], shots[0]["shot_id"]
    for i, s in enumerate(shots[:-1]):
        q = vlm.get(s["shot_id"], {})
        pp = plot.get(s["shot_id"], {})
        sc = round(W[0]*q.get("location_change",0) + W[1]*q.get("character_group_change",0) + W[2]*pp.get("plot_change",0))
        dur = s["end_ms"] - start
        if sc >= 50 and dur >= 30000:
            scenes.append({"start_shot":start_sid,"end_shot":s["shot_id"],"start_ms":start,"end_ms":s["end_ms"],"scene_score":sc})
            start, start_sid = shots[i+1]["start_ms"], shots[i+1]["shot_id"]
    scenes.append({"start_shot":start_sid,"end_shot":shots[-1]["shot_id"],"start_ms":start,"end_ms":shots[-1]["end_ms"],"scene_score":0})
    json.dump({"video_id":VID,"final_scenes":scenes}, open(final_out,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
    print(f"  Scenes: {len(scenes)}")
    for s in scenes:
        m1,s1=divmod(s["start_ms"],60000);m2,s2=divmod(s["end_ms"],60000)
        print(f"  {s['start_shot']}->{s['end_shot']}: [{int(m1)}:{s1/1000:04.1f}-{int(m2)}:{s2/1000:04.1f}] ({int((s['end_ms']-s['start_ms'])/1000)}s) score={s['scene_score']}")
...
def collect():
    ...
    with open(OUT / "08_pipeline_log.json", "w", encoding="utf-8") as f:
        json.dump({"video":VID,"duration_ms":shots[-1]["end_ms"],"shots":len(shots),"steps":log}, f, ensure_ascii=False, indent=2)
...
"""
    with open(OUT / "09_pipeline_report.md", "w", encoding="utf-8") as f:
        f.write(md)
    print(f"  Written: 09_pipeline_report.md")
